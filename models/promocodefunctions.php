<?php
    ...
    {
       static function get_promocode() {
            $con = mysqli_connect(SERVER, DB_USERNAME, DB_PASSWORD, DATABASE) or die(mysqli_connect_error());
            $seller_id=$_SESSION['user_id'];
            if($_SESSION['role']=='3')
            {
            $result = mysqli_query($con, "SELECT * FROM `ecom_promocodes` WHERE is_deleted='0' AND seller_id=$seller_id order by `id` DESC")or die(mysqli_error($con));
            }else{
            $result = mysqli_query($con, "SELECT * FROM `ecom_promocodes` WHERE is_deleted='0' order by `id` DESC")or die(mysqli_error($con));
            }
            mysqli_close($con);
            return $result;
        }
