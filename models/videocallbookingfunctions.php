<?php
    ...
    class VideoCallBookingFunctions
    {
       static function get_video_call_booking() {
            $con = mysqli_connect(SERVER, DB_USERNAME, DB_PASSWORD, DATABASE) or die(mysqli_connect_error());
            $result = mysqli_query($con, "SELECT * FROM `user_plans` JOIN `user_details` ON `user_plans`.`order_id`=`user_details`.`order_id` LEFT JOIN `voice_slot` ON `voice_slot`.`id`=`user_details`.`slot` WHERE `user_details`.`call_type`='video_call' order by `user_plans`.`id` DESC")or die(mysqli_error($con));
            mysqli_close($con);
            return $result;
        }
